...
export const getSevenDays = (startDate: Date = new Date()): Date[] => {
...
export const getEventDaySegment = (
  start: Date,
  end: Date,
  day: Date,
  ...
  const startMinutes = Math.round((segStart - dayStart) / MS_PER_MINUTE);
  const durationMinutes = Math.round((segEnd - segStart) / MS_PER_MINUTE);

  return {
    startMinutes,
    durationMinutes,
    continuesFromPrev: start.getTime() < dayStart,
    continuesToNext: end.getTime() > dayEnd,
  };
};
